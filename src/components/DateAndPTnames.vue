    ...
    <v-card-text>
      <v-timeline density="compact" align="start">
        ...
        <v-timeline-item dot-color="orange" size="x-small" v-for="([day, pts], i) in getMapWithDateAndPT(onset_dates, PT_names)" :key="day + '-' + i">
          <div class="mb-4">
            <strong>{{ day }}</strong>： <span v-if="ElapsedDays(vaccinated_date, day) != undefined">[接種から <v-chip variant="elevated" label size="small" color="blue-grey-lighten-5">{{ ElapsedDays(vaccinated_date, day) }}</v-chip> 日後]</span>
            <div class="pt-list">
        ...
        >
          <div class="mb-4">
            <div>
              <strong>{{ rDay }}</strong>： <span v-if="ElapsedDays(vaccinated_date, rDay) != undefined">[接種から <v-chip variant="elevated" label size="small" color="blue-grey-lighten-5">{{ ElapsedDays(vaccinated_date, rDay) }}</v-chip> 日後]</span>
              <div class="pt-list">
